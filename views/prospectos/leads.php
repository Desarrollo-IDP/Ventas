...
<div class="modal fade" id="modalImportar" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-lg"><div class="modal-content"><form id="formImportar" enctype="multipart/form-data" onsubmit="importarLeads(event)"><div class="modal-header"><h5 class="modal-title"><i class="fas fa-file-csv text-success me-2"></i>Importar empresas desde CSV</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><p class="text-muted">El archivo debe contener únicamente: <strong>Empresa, Número, Correo electrónico y Ubicación</strong>. Todos los registros se guardarán automáticamente con estado <strong>Lead</strong>.</p><div class="card bg-light border-0 mb-3 p-2"><div class="d-flex flex-wrap gap-2 align-items-center"><span class="small text-muted fw-bold me-2"><i class="fas fa-download me-1"></i> Plantillas CSV:</span><a href="../../plantilla_importacion_leads.csv?v=20260922-2" class="btn btn-sm btn-outline-primary" download><i class="fas fa-file-csv me-1"></i> Plantilla (.csv)</a> </div></div><input type="file" name="archivo" class="form-control" accept=".csv" required><small class="text-muted d-block mt-2">El número es suficiente como dato de contacto. Las filas repetidas por correo, número o empresa se omitirán automáticamente.</small><div id="resultadoImportacion" class="mt-3"></div></div><div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancelar</button><button class="btn btn-success"><i class="fas fa-upload me-1"></i> Importar archivo CSV</button></div></form></div></div></div>

<script>
document.addEventListener('DOMContentLoaded', () => {
    const nombre = document.querySelector('#modalLead [name="nombre"]');
    const cargo = document.querySelector('#modalLead [name="cargo_contacto"]');
    if (nombre) nombre.closest('.col-md-6')?.remove();
    if (cargo) cargo.closest('.col-md-6')?.remove();
    document.querySelectorAll('#modalLead h6').forEach((encabezado) => {
        if (encabezado.textContent.includes('Contacto primario')) {
            encabezado.textContent = 'Datos de contacto disponibles';
        }
    });
});
